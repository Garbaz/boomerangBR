...
use crate::{boomerang::Boomerang, game_state::GameState, traits::AsSfmlVector2};
...
impl Player {
    pub fn new(pos: Vec2) -> Self {
        ...
        Self { pos, vel: vec2(0., 0.), shape }
    }
    pub fn update(&mut self, boomerangs: &mut Vec<Boomerang>, dt: f32) {
        let key_dir = Player::get_key_dir();
        self.vel = key_dir * SPEED;
        self.pos = self.pos + self.vel * dt;
        self.throw_boomerang(boomerangs);
    }
    ...
    fn get_key_dir() -> Vec2 {
        ...
        let dir = vec2((right as i32 as f32) - (left as i32 as f32), (down as i32 as f32) - (up as i32 as f32));
        if dir != vec2(0., 0.) {
    ...
    fn throw_boomerang(&self, boomerangs: &mut Vec<Boomerang>) {
        boomerangs.push(Boomerang::new(vec2(100., 100.), vec2(1., 0.)))
    }
}
